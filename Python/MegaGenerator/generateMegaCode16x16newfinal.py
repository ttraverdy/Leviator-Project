...
##
## definitions for animations - various combinations - one is selected after
##

##
## for 10mm transducers
##
# The next tables defines in hexadecimal the action of each postion in object's movement for a Leviator using 10mm transducers

transducerAnimationParticleToParticle10mm = [
# This one defines the evolution of the phase of the 2 transduceurs of the middle of the 6 transducers used in one move
...
transducerAnimationParticleToNeighbor10mm = [
# This one defines the evolution of the phase of the 2 transduceurs of the old postion of the 6 transducers used in one move
...
transducerAnimationNeighborToParticle10mm = [
# This one defines the evolution of the phase of the 2 transduceurs of the new postion of the 6 transducers used in one move
...
transducerAnimationParticleToParticle16mm = [
# This one defines the evolution of the phase of the 2 transduceurs of the middle of the 6 transducers used in one move
...
transducerAnimationParticleToNeighbor16mm = [
# This one defines the evolution of the phase of the 2 transduceurs of the old postion of the 6 transducers used in one move
...
transducerAnimationNeighborToParticle16mm = [
# This one defines the evolution of the phase of the 2 transduceurs of the new postion of the 6 transducers used in one move
...
#different tables of hexadecimal values for transducers activation for movement using the amplitude modulation method, which isn't very trustworthy

transducerAnimationParticleToNeighborGOOD = [
...
transducerAnimationParticleToParticleGOOD = [
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff],
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff],
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff],
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff],
...
transducerAnimationNeighborToParticleGOOD = [
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
...
##
## Animations that do not change
##

# Tables of values of for keeping transducers off
...
## selection of the animation frames to use

transducerAnimationParticleToParticle = transducerAnimationParticleToParticle16mm
transducerAnimationParticleToNeighbor = transducerAnimationParticleToNeighbor16mm
transducerAnimationNeighborToParticle = transducerAnimationNeighborToParticle16mm
...
def writePortArrayStart():
    ...
    arrayFile.write("# define WAIT_LOT_STEADY(a) __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");  __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");__asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");__asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");\n")
    arrayFile.write("# define WAIT_LIT_STEADY(a) __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); \n")
    arrayFile.write("# define WAIT_LIT_STEADY_B(a) __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");\n")

    arrayFile.write("# define WAIT_LOT_ANIM(a) __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");  __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");__asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");__asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");;\n")
    arrayFile.write("# define WAIT_LIT_ANIM(a) __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");;\n")
    arrayFile.write("# define WAIT_LIT_ANIM_B(a) __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\"); __asm__ __volatile__ (\"nop\");\n")
    ...
    arrayFile.write("# define VAL_MUL_4_12_16          %d\n" % (4*12*MAX_ANIM_STEPS) )
    arrayFile.write("# define VAL_MUL_4_12_8           %d\n" % (2*12*MAX_ANIM_STEPS) )
    ...
    arrayFile.write("unsigned int maxIndex = %d;\n" % (MAX_ANIM_STEPS*4*16*4*12) )
    arrayFile.write("unsigned int maxIndex_1 = %d ;\n" % (MAX_ANIM_STEPS*4*16*4*12/4) )
    arrayFile.write("unsigned int maxIndex_2 = %d ;\n" % (MAX_ANIM_STEPS*4*16*4*12*2/4) )
    arrayFile.write("unsigned int maxIndex_3 = %d ;\n" % (MAX_ANIM_STEPS*4*16*4*12*3/4) )
    arrayFile.write("unsigned int maxIndex_4 = %d ;\n" % (MAX_ANIM_STEPS*4*16*4*12) )
    arrayFile.write("\n")
...
def writeSetupCode():
    ...
    arrayFile.write("\n")
    arrayFile.write("  byte animationStates[VAL_MUL_4_12_16];\n")
    arrayFile.write("  byte animationStatesSingle[VAL_MUL_4_12];\n")
    arrayFile.write("\n")
...
def writeLoopSteadyCode():
    ...
    for nFrame in xrange(0, MAX_FRAMES):
        ...
        if nFrame==2 or nFrame==9:
            arrayFile.write("    WAIT_LOT_STEADY() ; WAIT_LIT_STEADY_B();\n")
        else:
            arrayFile.write("    WAIT_LOT_STEADY() ; WAIT_LIT_STEADY();\n")
    arrayFile.write("\n")
...
def writeLoop2DCode():
    ...
    for animStep in xrange(0, MAX_ANIM_STEPS):
        ...
        for nFrame in xrange(0, MAX_FRAMES):
            ...
            if nFrame == 2 or nFrame == 9:
                arrayFile.write("    WAIT_LOT_ANIM() ; WAIT_LIT_ANIM_B();\n")
            else:
                arrayFile.write("    WAIT_LOT_ANIM() ; WAIT_LIT_ANIM();\n")
        arrayFile.write("}\n")
